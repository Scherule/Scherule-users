...
class UserControllerTest : AbstractControllerTest() {
    ...
    fun get_users_me_200() {
        ...
        this.mvc.perform(get("/api/users/me").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk).andExpect(content().string("""{"id":"red","email":"agatha@dummy.com","password":null,"firstName":"Agatha","lastName":"Someone","_links":{"self":{"href":"http://localhost/api/users/me"}}}"""))
    }
    ...
    fun get_users_200() {
        ...
        BDDMockito.given(userService.getSome(pageRequest)).willReturn(PageImpl(listOf(RED_USER, BLUE_USER), pageRequest, 111))
        this.mvc.perform(get("/api/users/?size=2&page=6").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk).andExpect(content().string("""{"_embedded":{"userModelList":[{"id":"red","email":"agatha@dummy.com","password":null,"firstName":"Agatha","lastName":"Someone"},{"id":"blue","email":"greg@dummy.com","password":null,"firstName":"Greg","lastName":"Someone"}]},"_links":{"first":{"href":"http://localhost/api/users/?page=0&size=2"},"prev":{"href":"http://localhost/api/users/?page=5&size=2"},"self":{"href":"http://localhost/api/users/?page=6&size=2"},"next":{"href":"http://localhost/api/users/?page=7&size=2"},"last":{"href":"http://localhost/api/users/?page=55&size=2"}},"page":{"size":2,"totalElements":111,"totalPages":56,"number":6}}"""))
    }
